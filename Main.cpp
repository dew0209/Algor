...
//差分一维二维
void init_diff_one(int a[],int b[],int n){for(int i = 1;i <= n;i++)b[i] = a[i] - a[i - 1];}
void init_diff_two(int a[][1010],int x1,int y1,int x2,int y2,int c){a[x1][y1] += c;a[x2 + 1][y1] -= c;a[x1][y2 + 1] -= c;a[x2 + 1][y2 + 1] += c;}

//lowbit
int lowbit(int u){int res = 0;for(int i = u;i;i -= i & -i)res++;return res;}
